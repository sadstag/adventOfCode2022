...
export const SOURCE_ABSOLUTE_LOCATION: Location = { x: 500, y: 0 };
...
const computeFrame = (paths: Path[]): { offset: Vector; size: Vector } => {
  ...
  // source must be taken in account
  offset.x = Math.min(offset.x, SOURCE_ABSOLUTE_LOCATION.x);
  offset.y = Math.min(offset.y, SOURCE_ABSOLUTE_LOCATION.y);
  max.x = Math.max(max.x, SOURCE_ABSOLUTE_LOCATION.x);
  max.y = Math.max(max.y, SOURCE_ABSOLUTE_LOCATION.y);
